...
| **A-04** | Gate names and sequence G0–G6 | 🔴 **Corrected** | P06 | Master Prompt §16 names a **different** sequence: G2 is Evidence Review and **G6** is Publication, not G4. The assumed names came from the pre-specification strategy document. Corrected in [ADR-0011](../adr/0011-governance-gate-model.md), superseding ADR-0005. §17 resolves authority by making it *configurable*, so SLA and role mapping are tenant configuration rather than missing values. |
...
| **A-07** | Multi-tenancy is logical — shared schema, row-level filtering | 🟢 **Confirmed as the default** | *(resolved)* | Confirmed by Master Prompt §9. The isolation mechanism is unchanged. Its *framing* changed: §10 requires residency to be able to override it per tenant, so [ADR-0010](../adr/0010-hybrid-tenancy-and-data-residency.md) supersedes ADR-0008. Shared schema is now the default, not the only model. |
...
| **A-18** | No foreign key from tenant-owned tables to `tenant` | 🟢 **Resolved** | *(P02 done)* | Exactly the revisit anticipated. `Tenant` moved to the platform control plane, so the foreign key no longer inverts the context map. All ten tenant-owned tables now carry `tenant_id → tenant.id` with `ON DELETE RESTRICT`, enforced by PostgreSQL rather than by review. |
| **A-19** | Geography semantics — jurisdiction vs physical residency | 🟢 **Resolved: both** | *(architecture done in P02)* | Master Prompt §10: geography means **both**, modelled separately. Jurisdiction is an ABAC attribute (`geography.jurisdiction`, shipped P02, hierarchy **not** evaluated — see A-33). Residency is four region assignments per tenant plus a mode, with strict tenants failing closed. **Architecture only**: actual regional deployment is future infrastructure work, and the platform must not claim residency is satisfied because the abstraction exists. See [ADR-0010](../adr/0010-hybrid-tenancy-and-data-residency.md). |
...
| **A-25** | Every tenant's objects share one bucket | 🟢 **Resolved** | *(P02 done)* | Buckets now resolve **per region**, and `stored_object.region_code` records where each object physically sits alongside its bucket. A single configured bucket behaves exactly as before. The seam did its job: one resolver changed, nothing else in the package. |
...
| **A-33** | Jurisdiction hierarchy is **stored but not evaluated** | 🟡 Open | A later commit | Medium. `Jurisdiction.parent_id` records containment, but nothing walks it: whether an EU-jurisdiction rule applies to a DE-jurisdiction subject is not specified in §10, and inventing that answer would put an unapproved legal interpretation into the authorization path. Adding traversal later is a resolver change, not a schema change. |
| **A-34** | Residency **change history** is not modelled | 🟡 Open | A later commit | Low. A tenant's regions are current values, not effective-dated rows. Changing them is a physical data-migration event recorded in the audit log, not a column update with history. If §37-style effective dating is required for residency, it is an additive table. |
| **A-35** | Residency **architecture** is implemented; regional **deployment** is not | 🟡 Open | Infrastructure | **High if misread.** P02 delivers tenant→region mapping, routing seams and fail-closed strict mode. It does **not** deliver a database in Frankfurt. A strict tenant is compliant only if its assigned region is genuinely deployed. The platform must never report residency as satisfied merely because the abstraction exists (Master Prompt §10). |
...
| Object storage | **Already seamed** (Commit 008): `SingleBucketResolver` is replaced by a residency-aware resolver, and `stored_object.bucket` is per row so existing objects are not stranded. This is the easy half. |

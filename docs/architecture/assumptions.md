...
| **A-19** | Geography semantics — jurisdiction, region or data residency | 🟡 **Open, deliberately undecided** | A later commit | **Medium-high, and the top structural risk.** Residency is not merely a filter: it constrains *where data may be stored*. Commit 008 built the storage half of the answer as a replaceable seam (`app/storage/locations.py`, A-25). The **relational** half is not solved and cannot be by a resolver — shared-schema tenancy (ADR-0008) puts every tenant's rows in one database. See the flagged section below. |
...
| **A-25** | Every tenant's objects share one bucket, separated by key prefix | 🟡 Open | A-19 | Medium, and **contained by design**. `app/storage/locations.py` resolves a bucket *per tenant* even though the default ignores the tenant, so a residency policy replaces one resolver and nothing else in the storage package. `stored_object.bucket` is stored per row, so objects written before a change stay readable. |
